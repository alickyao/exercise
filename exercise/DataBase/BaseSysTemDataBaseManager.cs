...
namespace cyclonestyle.DataBase
{
    ...
    public class BaseSysTemDataBaseManager
    {
        ...
        /// </summary>
        /// <returns></returns>
        internal static List<Combobox> RsGetHotelHotBrandList()
        {
            using (SysTemDataBaseDataContext context = new SysTemDataBaseDataContext(SqlConnection)) {
                string[] brandname = { "7天", "布丁", "汉庭", "锦江之星", "莫泰", "如家", "速8", "星程", "凯宾斯基", "千禧", "洲际", "万豪", "香格里拉", "雅高" };
                List<Combobox> rows = (from c in context.ElongHotel_Brands
                                       where brandname.Contains(c.ShortName)
                                       select new Combobox
                                       {
                                           text = c.ShortName,
                                           id = c.ShortName
        ...
        /// <param name="q"></param>
        /// <returns></returns>
        internal static List<Combobox> RsSearchHotelBrand(string q) {
            using (SysTemDataBaseDataContext context = new SysTemDataBaseDataContext(SqlConnection)) {
                List<Combobox> result = (from c in context.ElongHotel_Brands
                                         where SqlMethods.Like(c.ShortName, string.Format("%{0}%", q))
                                         orderby c.ShortName ascending
                                         select new Combobox
                                         {
        ...
        /// <param name="q"></param>
        /// <returns></returns>
        internal static List<HotelSearchItemModel> RsSearchHotelSItemList(string id, string q)
        {
            using (SysTemDataBaseDataContext context = new SysTemDataBaseDataContext(SqlConnection)) {
                List<HotelSearchItemModel> result = new List<HotelSearchItemModel>();
                //先检索品牌
                List<Combobox> Blist = RsSearchHotelBrand(q);
                if (Blist.Count > 0) {
                    foreach (Combobox b in Blist) {
                        result.Add(new HotelSearchItemModel {
                            id = b.text,
                            text = b.text,
                            itemType = EnumHotelSearchItemType.B
                        });
                    }
                }
                //然后检索地域信息
                SearchAreaInfoListRequestModel searchAreacondtion = new SearchAreaInfoListRequestModel()
                {
                    CityCode = id,
                    UsePageSize = true,
                    Page = 1,
                ...
                SearchAreaInfoListReplayModel AreaList = RsGetAreaInfoListByCityCode(searchAreacondtion);
                if (AreaList.total > 0) {
                    foreach (GeoAreaInfoModel a in AreaList.rows) {
                        EnumHotelSearchItemType type = EnumHotelSearchItemType.B;
                        switch (a.T) {
                            case EnumAreaType.L:
                                type = EnumHotelSearchItemType.L;
                                break;
                            case EnumAreaType.D:
                                type = EnumHotelSearchItemType.D;
                                break;
                            case EnumAreaType.C:
                                type = EnumHotelSearchItemType.C;
                                break;
                        }
                        result.Add(new HotelSearchItemModel()
                        {
                            id = type == EnumHotelSearchItemType.D ? a.id : a.text,
                            text = a.text,
                            itemType = type
                        });
                    }
        ...
        /// <summary>
        /// 根据艺龙酒店ID获取酒店详情，如果没有找到匹配的本地静态酒店信息则返回null
        /// </summary>
        /// <param name="id">酒店ID</param>
        /// <returns></returns>
        internal static ElongHotel_Hotel RsGetHotelStaticInfoByHotelId(string id)
        {
            using (SysTemDataBaseDataContext context = new SysTemDataBaseDataContext(SqlConnection)) {
                ElongHotel_Hotel dbhotel = context.ElongHotel_Hotel.SingleOrDefault(p => p.Id == id);
                return dbhotel;
            }
        }
        /// <summary>
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        internal static string RsGetHotelThunmNailUrl(string id)
        {
            using (SysTemDataBaseDataContext context = new SysTemDataBaseDataContext(SqlConnection)) {
                string url = string.Empty;
                var result = (from c in context.ElongHotel_HotelImages where c.HotelId == id && c.IsCoverImage == "True" select new { c.Locations }).Skip(0).Take(1).ToList();
                if (result.Count > 0) {
                    url = result[0].Locations;
                }
                return url;
            }
        }
        ...
        /// <param name="id">酒店ID</param>
        /// <returns></returns>
        internal static List<ElongHotelImageModel> RsGetElongHotelImageListByHotelId(string id)
        {
            using (SysTemDataBaseDataContext context = new SysTemDataBaseDataContext(SqlConnection)) {
                List<ElongHotelImageModel> ilist = (from c in context.ElongHotel_HotelImages
                                                    where c.HotelId == id
                                                    select new ElongHotelImageModel
                                                    {
                                                        size = c.Size,
                                                        type = c.Type,
                                                        url = c.Locations,
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        internal static List<ElongHotel_HotelRooms> RsGetElongHotelRoomsByHotelId(string id)
        {
            using (SysTemDataBaseDataContext context = new SysTemDataBaseDataContext(SqlConnection)) {
                List<ElongHotel_HotelRooms> result = (from c in context.ElongHotel_HotelRooms
                                               where c.HotelId == id
                                               orderby c.Id
                                               select c).ToList();
                return result;
        ...
        /// <param name="condtion"></param>
        /// <returns></returns>
        internal static int RsGetAirPortCodeCount(FlightAirPortInfoModel condtion)
        {
            using (SysTemDataBaseDataContext context = new SysTemDataBaseDataContext(SqlConnection)) {
                return (from c in context.Us_RsFlightAirport
                        where (c.code.ToLower() == condtion.code.ToLower())
                        && (string.IsNullOrEmpty(condtion.id) ? true : c.Us_RsFlightAirportId != condtion.id)
                        select c.Us_RsFlightAirportId).Count();
            }
        }
        ...
        /// <param name="condtion"></param>
        /// <returns></returns>
        internal static ReplayBase RsEditFlightAirPort(FlightAirPortInfoModel condtion)
        {
            using (SysTemDataBaseDataContext context = new SysTemDataBaseDataContext(SqlConnection)) {
                ReplayBase result = new ReplayBase();
                ...
                {
                    //新增
                    Us_RsFlightAirport dbair = new Us_RsFlightAirport() {
                        Us_RsFlightAirportId = rowid,
                        caption = condtion.caption,
                        city = condtion.city,
                        code = condtion.code.ToUpper(),
                        hot = condtion.hot,
                        isOtherPortofTheCity = condtion.isOtherPortofTheCity,
                        modifiedOn = DateTime.Now,
                        pinyin = condtion.pinyin.ToLower(),
                        PY = condtion.PY.ToUpper()
                    };
                    context.Us_RsFlightAirport.InsertOnSubmit(dbair);
                }
                else {
                    //编辑
                    rowid = condtion.id;
                    Us_RsFlightAirport dbair = context.Us_RsFlightAirport.Single(p => p.Us_RsFlightAirportId == condtion.id);
                    dbair.caption = condtion.caption;
                    dbair.city = condtion.city;
                    dbair.code = condtion.code.ToUpper();
                    dbair.hot = condtion.hot;
                    dbair.isOtherPortofTheCity = condtion.isOtherPortofTheCity;
                    dbair.pinyin = condtion.pinyin.ToLower();
                    dbair.PY = condtion.PY.ToUpper();
                    dbair.modifiedOn = DateTime.Now;
                }
                context.SubmitChanges();
                return result;
            }
        ...
        /// <param name="condtion"></param>
        /// <returns></returns>
        internal static List<FlightAirPortInfoModel> RsGetFlightAirPortList(GetFlightAirPortListRequestModel condtion)
        {
            using (SysTemDataBaseDataContext context = new SysTemDataBaseDataContext(SqlConnection)) {
                string sorttype = string.Empty;
                switch (condtion.sorttype) {
                    case EnumSortOrderType.按时间升序:
                        sorttype = "modifiedOn asc";
                        break;
                    case EnumSortOrderType.按时间降序:
                        sorttype = "modifiedOn desc";
                        break;
                    case EnumSortOrderType.按排序号升序:
                        sorttype = "hot asc,city asc";
                        break;
                    case EnumSortOrderType.按排序号降序:
                        sorttype = "hot desc,city asc";
                        break;
                ...
                List<FlightAirPortInfoModel> result = (from c in context.Us_RsFlightAirport
                                                       where (condtion.showOtherAirport? true :c.isOtherPortofTheCity == false)
                                                       select new FlightAirPortInfoModel
                                                       {
                                                           caption = c.caption,
                                                           city = c.city,
                                                           code = c.code,
                                                           hot = c.hot,
                                                           id = c.Us_RsFlightAirportId,
                                                           isOtherPortofTheCity = c.isOtherPortofTheCity,
                                                           modifiedOn = c.modifiedOn,
                                                           pinyin = c.pinyin,
                                                           PY = c.PY
        ...
        /// <param name="Id"></param>
        /// <returns></returns>
        internal static ReplayBase RsDelFlightAirPortById(string Id)
        {
            using (SysTemDataBaseDataContext context = new SysTemDataBaseDataContext(SqlConnection)) {
                Us_RsFlightAirport c = context.Us_RsFlightAirport.Single(p => p.Us_RsFlightAirportId == Id);
                context.Us_RsFlightAirport.DeleteOnSubmit(c);
                context.SubmitChanges();
                return new ReplayBase() {
                    ReturnCode = EnumErrorCode.Success,
                    ReturnMessage = Id
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        internal static FlightAirPortInfoModel RsGetFlightAirPortInfoById(string id)
        {
            using (SysTemDataBaseDataContext context = new SysTemDataBaseDataContext(SqlConnection)) {
                Us_RsFlightAirport c = context.Us_RsFlightAirport.Single(p => p.Us_RsFlightAirportId == id);
                FlightAirPortInfoModel info = new FlightAirPortInfoModel() {
                    caption = c.caption,
                    city = c.city,
                    code = c.code,
                    hot = c.hot,
        ...
        /// <param name="condtion"></param>
        /// <returns></returns>
        internal static int RsGetAirCompanyCodeCount(FlightAirCompanyInfoModel condtion)
        {
            using (SysTemDataBaseDataContext context = new SysTemDataBaseDataContext(SqlConnection)) {
                return (from c in context.Us_RsFlightAirCompany
                        where (c.code.ToLower() == condtion.code.ToLower())
                        && (string.IsNullOrEmpty(condtion.id) ? true : condtion.id != c.Us_RsFlightAirCompanyId)
                        select c.Us_RsFlightAirCompanyId).Count();
            }
        }
        ...
        /// <param name="condtion"></param>
        /// <returns></returns>
        internal static ReplayBase RsEditFlightAirCompany(FlightAirCompanyInfoModel condtion)
        {
            using (SysTemDataBaseDataContext context = new SysTemDataBaseDataContext(SqlConnection)) {
                string rowid = Guid.NewGuid().ToString();
                if (string.IsNullOrEmpty(condtion.id))
                {
                    //新增
                    Us_RsFlightAirCompany dbc = new Us_RsFlightAirCompany() {
                        Us_RsFlightAirCompanyId = rowid,
                        caption = condtion.caption,
                        code = condtion.code.ToUpper(),
                        company = condtion.company,
                        imgFilePath = condtion.imgFilePath,
                        sort = condtion.sort,
                        modifiedOn = DateTime.Now
                    };
                    context.Us_RsFlightAirCompany.InsertOnSubmit(dbc);
                }
                else {
                    //编辑
                    rowid = condtion.id;
                    Us_RsFlightAirCompany dbc = context.Us_RsFlightAirCompany.Single(p => p.Us_RsFlightAirCompanyId == condtion.id);
                    dbc.caption = condtion.caption;
                    dbc.code = condtion.code.ToUpper();
                    dbc.company = condtion.company;
                    dbc.imgFilePath = condtion.imgFilePath;
                    dbc.sort = condtion.sort;
                    dbc.modifiedOn = DateTime.Now;
                }
                context.SubmitChanges();
                return new ReplayBase() {
                    ReturnCode = EnumErrorCode.Success,
        ...
        /// <param name="condtion"></param>
        /// <returns></returns>
        internal static List<FlightAirCompanyInfoModel> RsGetFlightAirCompanyList(GetFlightAirCompanyListRequestModel condtion)
        {
            using (SysTemDataBaseDataContext context = new SysTemDataBaseDataContext(SqlConnection)) {
                string sorttype = string.Empty;
                switch (condtion.sorttype)
                {
                    ...
                    case EnumSortOrderType.按时间降序:
                        ...
                        break;
                    case EnumSortOrderType.按排序号升序:
                        sorttype = "sort asc,caption asc";
                        break;
                    case EnumSortOrderType.按排序号降序:
                        sorttype = "sort desc,caption asc";
                        break;
                    case EnumSortOrderType.按标题升序:
                        sorttype = "caption asc";
                        break;
                    case EnumSortOrderType.按标题降序:
                        sorttype = "caption desc";
                        break;
                }
                List<FlightAirCompanyInfoModel> result = (from c in context.Us_RsFlightAirCompany
                                                          select new FlightAirCompanyInfoModel
                                                          {
                                                              caption = c.caption,
                                                              code = c.code,
                                                              company = c.company,
                                                              id = c.Us_RsFlightAirCompanyId,
                                                              imgFilePath = c.imgFilePath,
                                                              modifiedOn = c.modifiedOn,
                                                              sort = c.sort
                                                          }).OrderBy(sorttype).ToList();
                return result;
        ...
        /// <returns></returns>
        internal static FlightAirCompanyInfoModel RsGetFlightCompanyInfoById(string id)
        {
            using (SysTemDataBaseDataContext context = new SysTemDataBaseDataContext(SqlConnection)) {
                Us_RsFlightAirCompany c = context.Us_RsFlightAirCompany.Single(p => p.Us_RsFlightAirCompanyId == id);
                FlightAirCompanyInfoModel info = new FlightAirCompanyInfoModel() {
                    caption = c.caption,
                    code = c.code.ToUpper(),
                    company = c.company,
                    id = c.Us_RsFlightAirCompanyId,
        ...
        /// <param name="id">Id</param>
        /// <returns></returns>
        internal static ReplayBase RsDelFlightCompanyById(string id)
        {
            using (SysTemDataBaseDataContext context = new SysTemDataBaseDataContext(SqlConnection)) {
                Us_RsFlightAirCompany c = context.Us_RsFlightAirCompany.Single(p => p.Us_RsFlightAirCompanyId == id);
                context.Us_RsFlightAirCompany.DeleteOnSubmit(c);
                context.SubmitChanges();
                return new ReplayBase() {
                    ReturnCode = EnumErrorCode.Success,
                    ReturnMessage = id
